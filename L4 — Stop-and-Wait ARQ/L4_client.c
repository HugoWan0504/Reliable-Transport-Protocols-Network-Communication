...
int main(int argc, char const *argv[]) {
  ...
  /* Number of packets */
  /* TODO 0: change 5 to atoi(argv[2])*/
  pkt_num = atoi(argv[2]);
  if (pkt_num <= 0) {
  ...
  /* TODO 1: Change this initial hello message into a Packet structure
   * In this packet, set flag to PKT_REQ and value to be the pkt_num
   * So then server knows how many packets client is requesting
   */
  Packet pkt_send;
  ...
  while (pkt_num > 0) {
    ...
    if (pkt_recv.flag == PKT_DATA) {
      ...
      pkt_num--;
      /* Send ACK */
      /* TODO 2: Change this ACK message into a Packet structure
       * In this packet, set flag to PKT_ACK
       * And the value to be the value from received packet
       * So then server knows client successfully received the packet
       */
      Packet pkt_ack;
